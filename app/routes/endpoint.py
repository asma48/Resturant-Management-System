...
from app.schemas.data_model import MenuBase, PlaceOrder, OrderResponse, Order_Details, MenuResponse, ItemsBase, Order_Update, CategoryResponse, EmployeData, UpdateEmploye, EmployeDelete, MenuUpdate,CategoryUpdate
...
employe_router  = APIRouter(
    prefix= "/employe",
    tags = ["Employe Account Management"]
)
...
@employe_router.get('/', response_model=EmployeData)
def get_employe(current_employe: Annotated[dict, Depends(get_current_employe)]):

    return current_employe


@employe_router.put('/update/{id}')
...
@employe_router.delete("/delete employe")
...
@employe_router.get("/list_of_employes")
